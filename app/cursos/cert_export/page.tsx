...
const CertificatesTable: React.FC<{ certificates: CertificatesList }> = ({ certificates }) => {
    return (
...
export default function Home() {
    ...
    const [companyCertificates, setCompanyCertificates] = useState<CertificatesList>([]);
    ...
    useEffect(() => {
        const obtenerCertificados = async () => {
            try {
                ...
                const digitalCerts = certificates.filter(cert => cert.type === 'certificadoDigital');
                const physicalCerts = certificates.filter(cert => cert.type === 'certificadoFisico');
                const companyCerts = certificates.filter(cert => cert.type === 'certificadoOnly');
    ...
    const exportarPDF = () => {
        const allCertificates: Certificate[] = [...digitalCertificates, ...physicalCertificates, ...companyCertificates];
        ...
        allCertificates.forEach((certificate, index) => {
            ...
            pdf.addImage(certificate.certificateDataURL, 'JPEG', 0, 0, width, height, '', 'SLOW'); // Agrega la imagen al tamaño de la página con calidad rápida
            pdf.save(`certificado_${certificate.ownerName}_${certificate.type}_${index + 1}.pdf`); // Guarda el PDF con un nombre único basado en el nombre del participante, su tipo y el índice
        });
    };
        ...
        <div className="bg-black h-screen overflow-hidden">
            ...
            <div className="bg-black flex h-full">
                ...
                <div className=" flex w-full ">
                    {/* Sidebar */}
                    <div className="w-1/4 p-4 bg-gray-800 text-white mt-4 h-full rounded-r-xl">
                        <ul>
                            <li>
                                <button className="w-full btn bg-green-500 text-white hover:bg-green-400 mt-2" onClick={exportarPDF}>
                                    Exportar en PDF
                                </button>
                            </li>
                            <li>
                                <div className="join grid grid-cols-2 mt-3">
                                    ...
                                    <Link href="/" passHref legacyBehavior>
                                        <button className="join-item btn-info btn-disabled text-white btn">Siguiente</button>
                                    </Link>
                                </div>
                            </li>
                        </ul>
